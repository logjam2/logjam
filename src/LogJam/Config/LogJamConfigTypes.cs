...
namespace LogJam.Config
{
	...
	/// <summary>
	/// Manages all the configuration types that are used during config file serialization and deserialization - needed so that CLR types can be determined from untyped JSON.
	/// When new classes are created that will be used in LogJam config files, they need to be registered via the <see cref="Register"/> method.
	/// </summary>
	public static class LogJamConfigTypes
	{
		...
		private static readonly Type[] s_standardConfigTypes = {
																   ...
																   typeof(ConsoleTraceWriterConfig)
		                                                       };
			...
			if (baseType.IsGenericType)
			{
				...
				{	// Try direct ordered application of type parameters from the base type to the config type
					...
					foreach (Type gtdMatch in gtdMatches)
					{
						...
						catch {} // This is valid; .MakeGenericType() could fail.
					}
				}
			}
