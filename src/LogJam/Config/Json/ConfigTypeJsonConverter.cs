// -----------------------------------------------------------------------
// <copyright file="ConfigTypeJsonConverter.cs" company="PrecisionDemand">
...
namespace LogJam.Config.Json
{
	...
	internal sealed class ConfigTypeJsonConverter : JsonConverter
	{
		...
		internal ConfigTypeJsonConverter(Type baseType)//, JsonSerializer innerSerializer)
		{
			...
			_typesWithProperty = _possibleTypes.SelectMany(type => type.GetProperties().Select(property => new { type, property }))
				.ToLookup(a => a.property.Name, a => a.type);
			// Create TypeHint -> type lookup
			_typesWithTypeHint = _possibleTypes.SelectMany(type => JsonTypeHintAttribute.For(type).Select(jsonHint => new { jsonHint, type}))
